...
# The following contains auxiliary functions to conduct additional simulation 
...
oneDat <- function(n, k, beta1, beta2, b, gamma01, gamma02, noninf=NULL){
  ...
  eta1 <- sapply(1:k, function(j) gamma01[j]+beta1[1]*sin(x1)+beta1[2]*(cos(x2)-0.6065)+beta1[3]*x5+beta1[4]*x6)
  eta2 <- sapply(1:k, function(j) gamma02[j]+beta2[1]*sin(x3)+beta2[2]*(cos(x4)-0.6065)+beta2[3]*x7+beta2[4]*x8) 
...
final_fit <- function(stab, q){
  ...
  sel  <- strsplit(sel,"[.]")
  selvar <- sapply(sel, `[[`, 1)
  seleta <- as.numeric(substr(sapply(sel, `[[`, 2),4,4))
  smooth <- substr(sapply(sel, `[[`, 1),3,3)=="s"
  selvar[smooth] <- paste0("sm.ps(", selvar[smooth],", outer.ok = T)")
  forml <- paste0(unique(selvar[!smooth]), collapse="+")
  forms <- paste0(unique(selvar[smooth]), collapse="+")
  if(forms==""){
    formf  <- formula(paste0("cbind(e0,e1,e2)~", forml))
  } else{
    formf  <- formula(paste0("cbind(e0,e1,e2)~", forml, "+", forms))
  }
  clist  <- vector(mode="list", length=length(unique(selvar))+1)
  names(clist) <- c("(Intercept)", unique(selvar[!smooth]), unique(selvar[smooth]))
  clist[[1]] <- diag(2)
  for(i in names(clist)[-1]){
    if(sum(i==selvar)==2){
      clist[[i]] <- diag(2)
    } else{ 
      if(seleta[i==selvar]==1){
        clist[[i]] <- rbind(1,0)
      } else{
        clist[[i]] <- rbind(0,1)
      }
