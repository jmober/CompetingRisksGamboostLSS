...
# Berger et. al (2019) and the functions implemented in "functionAc.R".

## Output 
# List with five elements (methods 0, 2 - 6)
...
# List with six elements (methods 1, 7)
# - predictive log-likelihood value 
# - C-index value 
# - matrix of time-dependent prediction error 
# - integrated prediction error value 
# - coefficients of the fitting GB model 
# - C-index value without accounting for ties
...
one_sim <- function(seed, n, k, b, noninf, mmax, method){
  ...
  source("./Simulation/functionAc.R")
  
  # set coefficients
  beta1 <- c(1.25, -1.8, 0.6, -0.6)
  beta2 <- c(1.25, -1.8, 0.6, -0.6)
  ...
  # variables for smooth base-learners
  namess <- paste0("x",1:8)
  datLong <- cbind(datLong, datLong[,namess])
  names(datLong)[(ncol(datLong)-(length(namess)-1)):ncol(datLong)] <- paste0(namess,"s")
  datLong_val <- cbind(datLong_val, datLong_val[,namess])
  names(datLong_val)[(ncol(datLong_val)-(length(namess)-1)):ncol(datLong_val)] <- paste0(namess,"s")
  datLong_test <- cbind(datLong_test, datLong_test[,namess])
  names(datLong_test)[(ncol(datLong_test)-(length(namess)-1)):ncol(datLong_test)] <- paste0(namess,"s")
  datLong_pred <- cbind(datLong_pred, datLong_pred[,namess])
  names(datLong_pred)[(ncol(datLong_pred)-(length(namess)-1)):ncol(datLong_pred)] <- paste0(namess,"s")
  ...
  if(method==1){ 
    ...
    form00  <- paste0("bols(", names, ",intercept=F)", collapse="+")
    form0 <- formula(paste0("y~bols(timeInt, intercept=T, contrasts.arg='contr.dummy')+", form00))
    form10 <- paste0("bbs(", paste0(namess,"s"),", center=T, df=1)", collapse="+")
    form1 <- formula(paste0("y~bols(timeInt, intercept=T, contrasts.arg='contr.dummy')+", form00, "+",form10))
    ...
    rm(mod1)
    to_return <- list(ll1,C1H,PE1,IPE1,coef(modopt1),C1)
  }
  ...
  if(method==2){
    ...
    form00  <- paste0("bols(", names, ",intercept=F) %O% bols(e, contrasts.arg='contr.dummy')", collapse="+")
    form0 <- formula(paste0("y~bols(timeInt, intercept=T, contrasts.arg='contr.dummy')  %O% bols(e, contrasts.arg='contr.dummy')+", form00))
    form20 <- paste0("bbs(", paste0(namess,"s"),", center=T, df=1) %O% bols(e, contrasts.arg='contr.dummy', df=1)", collapse="+")
    form2 <- formula(paste0("y~bols(timeInt, intercept=T, contrasts.arg='contr.dummy')  %O% bols(e, contrasts.arg='contr.dummy')+", form00, "+",form20))
  ...
  if(method==7){
    ...
    form00  <- paste0("bols(", names, ",intercept=F)", collapse="+")
    form0 <- formula(paste0("y~bols(timeInt, intercept=T, contrasts.arg='contr.dummy')+", form00))
    form70 <- paste0("bbs(", paste0(namess,"s"),", center=T, df=1)", collapse="+")
    form7 <- formula(paste0("y~bols(timeInt, intercept=T, contrasts.arg='contr.dummy')+", form00, "+",form70))
    ...
    modopt7 <- try(vgam(sel7[[1]], data=datLong, family=multinomial(parallel=FALSE, refLevel=1), constraints=sel7[[2]],
                        control=vgam.control(checkwz=F, Maxit.outer=1)))
    if(class(modopt7)!="try-error"){
      lambda_hat7  <- predict(modopt7, datLong_test, type="response")
      ...
      rm(mod7)
      to_return <- list(ll7,C7H,PE7,IPE7,coef(modopt7, matrix=T),C7)
    } else{
      to_return <- list(NA,NA,NA,NA,NA)
    }
  }
...
# The results of additional simulation scenario (c) for the seven methods 
# GB logL, GB StabS, GBg, PL, PLg, Tree, Null and True are stored in 
# "./raw_results/simulationAc.rda" (as a list with 2400 elements). 
...
# corresponding results with mmax=12000 is stored in 
res[[404]]
...
# library("batchtools")
# 
# r <- makeRegistry(file.dir="./Simulation/simulationAc")
# # r <- loadRegistry(file.dir="./Simulation/simulationAc")
# 
# args <- expand.grid(seed=c(1:100)+160920, 
#                     n=250,
#                     k=10,
...
#                     mmax=15000, 
#                     method=c(0:6))
# args$mmax[args$noninf==200] <- 11000
# args$mmax[args$noninf==1000] <- 7000
# 
