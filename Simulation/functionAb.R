...
oneDat <- function(n, k, beta1, beta2, b, gamma01, gamma02, noninf=NULL){
  ...
  mu <- rep(0,8+noninf)
  Sigma <- matrix(0.4, nrow=8+noninf, ncol=8+noninf)+ diag(0.6, nrow=8+noninf, ncol=8+noninf)
  
  X <- mvrnorm(n, mu, Sigma)
  colnames(X) <-  paste0("x",1:(8+noninf))
  
  eta1 <- sapply(1:k, function(j) gamma01[j]+beta1[1]*X[,"x1"]+beta1[2]*X[,"x2"]+beta1[3]*X[,"x5"]+beta1[4]*X[,"x6"])
  eta2 <- sapply(1:k, function(j) gamma02[j]+beta2[1]*X[,"x3"]+beta2[2]*X[,"x4"]+beta2[3]*X[,"x7"]+beta2[4]*X[,"x8"]) 
...
final_fit <- function(stab, q){
  ...
  formf  <- formula(paste0("cbind(e0,e1,e2)~",paste0(unique(selvar), collapse="+")))
  clist  <- vector(mode="list", length=length(unique(selvar))+1)
  names(clist) <- c("(Intercept)", unique(selvar))
  clist[[1]] <- diag(2)
