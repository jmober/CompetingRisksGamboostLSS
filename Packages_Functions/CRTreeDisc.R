...
# nfolds: number of folds when using tuning with "ll"
...
CRTreeDisc <- function(formula, data, tuning=c("AIC", "BIC", "ll"),
                       ...
                       eventColumns, minimal_ns, nfolds=5,
                       trace = FALSE){
  ...
  stratified <- function(df, group, size) {
    ...
    strat <- sampling:::strata(temp, stratanames = names(temp[group]),
  ...
  if(tuning == "ll"){
    for(cv in 1:nfolds){
      ...
      set.seed(cv)
      trainShort5 <- stratified(dataShortAll, timeColumn, 1/nfolds*(nfolds-1))
